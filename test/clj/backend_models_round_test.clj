...
(def sample-pile [{:type :pip, :value 6, :points 0.5, :suit :clubs}
...
(deftest calc-excuse-test
  (testing "with I.O.U should get a card from the other player"
    (let [{t-score :taker/score
           d-score :defenders/score}
          (sut/calculate-scores {:pile (conj sample-pile cards.data/iou)
                                 :bid p1-bid
                                 :bonuses p1-bonuses}
                                {:uids #{(:id player.data/player-two)
                                         (:id player.data/player-three)
